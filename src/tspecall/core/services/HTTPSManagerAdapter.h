...
public:
    ...
    void onDestroy(); // Application is destroyed.
    void sendRequestAsync1(string strURL, string dataBody, string strHttpMethod = "POST");
    void sendRequestAsync2(string strURL, string dataBody, bool needAuthenServer = false, string strHttpMethod = "POST");
    void registerService();
    RespInfo_t* getRespData();
    void clearRspData();
    std::string getTelNo();
    void setTelNo(std::string telNo);
    void onReceivedData(const android::sp<sl::Message>& message);

private:
    uint32_t sendRequest(string strURL, string dataBody, HttpData_t& respData,bool needAuthenServer = false, string strHttpMethod = "POST");
    void sendRecvThreadFunc(string strURL, string strPayload, bool needAuthenServer = false, string strHttpMethod = "POST");
#ifdef REGION_PCB26
    void setSecurityEvent(const CURLcode& iResultCode);
#endif
private:
    void initHttpData(HttpData_t *httpData);
